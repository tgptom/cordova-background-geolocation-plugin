...
| `url`                     | `String`          | all          | Server url where to send HTTP POST with recorded locations. HTTPS is required by default. **@see** [HTTP locations posting](#http-locations-posting)                                                                                                                                                                                               | all         |                            |
| `syncUrl`                 | `String`          | all          | Server url where to send fail to post locations. HTTPS is required by default. **@see** [HTTP locations posting](#http-locations-posting)                                                                                                                                                                                                          | all         |                            |
| `allowHttp`               | `Boolean`         | all          | Allows cleartext HTTP for `url` and `syncUrl`. Keep disabled for production and only enable in controlled development environments.                                                                                                                                                                                                                  | all         | false                      |
...
| `notificationPermissionGranted` | `Boolean` | Android 13+ notification runtime permission state (always true on older versions/iOS) |
